...
namespace wServer.realm
{
    public class ChatManager
    {
        ...
        public void Say(Player src, string text)
        {
            ...
            {
                Name = (src.Client.Account.Rank >= 5 ? "#[Founder]" : src.Client.Account.Rank >= 4 ? "#[Co-Founder]" : src.Client.Account.Rank >= 3 ? "#[Admin]" : src.Client.Account.Rank >= 2 ? "@[Modo]" : src.Client.Account.Rank >= 1 ? "#[Donor]" : "") + src.Name,
                ObjectId = src.Id,
                Stars = src.Stars,
                BubbleTime = 10,
                Recipient = "",
