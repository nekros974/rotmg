        ...
        public Database(string host, string database, string user, string password)
        {
            ...
            _con = new MySqlConnection(
                String.Format("Server={0};Database={1};uid={2};password={3};convert zero datetime=True;",
                    host, database ?? "rotmgprod", user ?? "root", password ?? "azerty"));
            _con.Open();
...
AND characters.charId=death.chrId;";
        ...
        public Account Register(string uuid, string password, bool isGuest, XmlData data)
        {
            ...
            if (success)
            {
                ...
                cmd = CreateQuery();
                cmd.CommandText = "INSERT INTO pets(accId, petId, objType, skinName, skin, family, rarity, maxLevel, abilities, levels, xp) VALUES(@accId, 1, 32577, '{pets.Golden_Sheepdog_Skin}', 32829, 0, 3, 90, '405, 408, 402', '1,1,1', '0,0,0');";
                cmd.Parameters.AddWithValue("@accId", accId);
                cmd.ExecuteNonQuery();
